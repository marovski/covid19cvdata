...
#             covid19cv$local <- gsub("Maio", "Vila do Maio", covid19cv$local)
            ...
            #get cases by cities
            covid19cv_concelhos<- covid19cv %>%  dplyr::mutate(Data = as.Date(data)) %>%
              tidyr::complete(Data = seq.Date(min(Data), Sys.Date(), by="day"),concelho) %>%
            ...
            covid19cv_concelhos<-covid19cv_concelhos %>% dplyr::group_by(concelho)%>% dplyr::mutate(confirmados_acumulados=cumsum(confirmados))

            #Ative positive cases
            covid19cv_concelhos<-covid19cv_concelhos %>% dplyr::group_by(concelho)%>% dplyr::mutate(c_ativos_acumulados=cumsum(confirmados-recuperados-obitos-evacuados))

            #Get lat & long
            x<-covid19cv %>% dplyr::select(concelho,lat,long,ilha)
            x<-unique(x)

            covid19cv_concelhos <- dplyr::left_join(covid19cv_concelhos,x, by = "concelho")
            covid19cv_concelhos<-covid19cv_concelhos[,c(1,2,9,10,11,3,4,5,6,7,8)]
